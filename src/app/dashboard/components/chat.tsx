...
const ChatComponent: React.FC<ChatComponentProps> = ({ children, mood, weatherInfo, locationInfo }) => {
    ...
    useEffect(() => {
        if (mood && weatherInfo) {
            fetchActivityRecommendation(mood, weatherInfo, locationInfo);
        }
    }, [mood, weatherInfo, locationInfo]);

    const fetchActivityRecommendation = async (mood: string, weather: any, location: string) => {
        try {
